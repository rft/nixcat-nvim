...
local function ensure_runtime_dir()
  ...
  local function is_writable(dir, create)
    dir = normalize(dir)
    if not dir then
      return false
    end

    if create and vim.fn.isdirectory(dir) == 0 then
      pcall(vim.fn.mkdir, dir, 'p', tonumber('700', 8))
    end

    local testfile = dir .. '/.nvim-perm-check'
    local fd = uv.fs_open(testfile, 'w', tonumber('600', 8))
    if not fd then
      return false
    end
  ...
  if is_writable(vim.env.XDG_RUNTIME_DIR, false) then
    return
  end

  local home = vim.env.HOME or vim.loop.os_homedir() or '.'
  local cwd = vim.loop.cwd()
  local user = vim.env.USER or 'nvim'
  local candidates = {
    { dir = (vim.fn.stdpath('state') or '') .. '/run', set_env = true },
    { dir = (vim.fn.stdpath('cache') or '') .. '/nvim-runtime', set_env = true },
    { dir = (vim.fn.stdpath('data') or '') .. '/nvim-runtime', set_env = true },
    { dir = home .. '/.cache/nvim-runtime', set_env = true },
    { dir = home .. '/.local/state/nvim/run', set_env = true },
    { dir = cwd and (cwd .. '/.nvim-runtime') or nil, set_env = true },
    { dir = '/tmp/nvim-runtime-' .. user, set_env = true },
  }

  local tried = {}
  for _, candidate in ipairs(candidates) do
    local dir = normalize(candidate.dir)
    if dir and not tried[dir] and is_writable(dir, true) then
      if candidate.set_env then
  ...
  vim.schedule(function()
    vim.notify('Neovim could not find a writable XDG_RUNTIME_DIR; RPC-based plugins may fail.', vim.log.levels.WARN)
  end)
end

ensure_runtime_dir() -- Ensure RPC server directory stays writable for plugins like dbee
...
require('nixCatsUtils.lazyCat').setup(nixCats.pawsible { 'allPlugins', 'start', 'lazy.nvim' }, {
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>ss', builtin.current_buffer_fuzzy_find, { desc = '[S]earch in current file' })
